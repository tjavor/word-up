...
function addNewWordSubmission(word) {
    ...
    for (var i = 0, len = model.wordSubmissions.length; i < len; i++) {
        var submission = model.wordSubmissions[i];
...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            //for (var i = 0, len = wordSubmissions.length; i < len; i++) {
            //    var item = wordSubmissions[i];
            //    item.isRealWord = theAnswer;
            //}
...
function isDisallowedLetter(letter) {
    ...
    return model.allowedLetters.indexOf(letter) < 0;
...
function containsOnlyAllowedLetters(word) {
    ...
    var containsOnlyAllowedLetter = disallowedLettersInWord(model.currentAttempt);
    if (containsOnlyAllowedLetter.length < 1) {
    return true;
}
}
